    ...
    with ctx.progress_scope(100, "Exploring tree-sitter queries") as progress:
        try:
            ...
            project_info = mcp.tools.register_project_tool(
                path=project_path, name=project_name, description="Project for query exploration"
            )
...
def run_queries(
    project_name: str, language: str, file_path: str, templates: Dict[str, Any], progress: Any
) -> Dict[str, Any]:
    """
    ...
    """
    results_data = {"language": language, "file": file_path, "queries": {}}
    ...
    try:
        ...
        if language in templates and templates[language]:
            ...
            results = mcp.tools.run_query(
                project=project_name, query=template["query"], file_path=file_path, max_results=10
            )

            # Save results data
            results_data["queries"][template_name] = {"query": template["query"], "results": results}
        ...
        if language in templates and len(templates[language]) >= 2:
            ...
            compound = mcp.tools.build_query(language=language, patterns=template_names, combine="or")

            # Run the compound query
            compound_results = mcp.tools.run_query(
                project=project_name, query=compound["query"], file_path=file_path, max_results=10
            )
            ...
            if results and compound_results:
                ...
                if new_results:
                    print("New matches from compound query:")
        ...
        if pattern:
            ...
            pattern_results = mcp.tools.run_query(
                project=project_name, query=pattern, file_path=file_path, max_results=5
            )
        ...
        if target_langs and pattern:
            for target_lang in target_langs:
                try:
                    # Adapt the query
                    adapted = mcp.tools.adapt_query(query=pattern, from_language=source_lang, to_language=target_lang)
        ...
        try:
            ...
            for _i, (node_type, description) in enumerate(list(node_types.items())[:7]):
