...
def node_to_dict(
    ...
    if source_bytes and include_text:
        try:
            result["text"] = source_bytes[node.start_byte:node.end_byte].decode('utf-8', errors='replace')
...
def find_node_at_position(
    ...
    if not (
        root_node.start_point <= point <= root_node.end_point
    ):
        return None
    ...
    while cursor.goto_first_child():
        ...
        if not (
            cursor.node.start_point <= point <= cursor.node.end_point
        ):
            cursor.goto_parent()
