...
def extract_symbols(
    ...
    try:
        ...
        raise ValueError(f"Error extracting symbols from {file_path}: {e}")
...
def analyze_project_structure(
    ...
    for current_dir, dirs, files in os.walk(root):
        ...
        dirs[:] = [d for d in dirs if not d.startswith('.') and 
                   d not in CONFIG.security.excluded_dirs]
    ...
    if scan_depth > 0:
        ...
        for language, count in languages.items():
            extensions = [ext for ext, lang in language_registry._language_map.items() 
                         if lang == language]
...
def find_dependencies(
    ...
    try:
        ...
        raise ValueError(f"Error finding dependencies in {file_path}: {e}")
...
def analyze_code_complexity(
    ...
    try:
        ...
        if language in comment_starters:
            ...
            comment_lines = sum(1 for line in lines if line.strip().startswith(comment_start))
        ...
        complexity_nodes = {
            ...
            "javascript": ["if_statement", "for_statement", "while_statement", "try_statement"],
            "typescript": ["if_statement", "for_statement", "while_statement", "try_statement"],
            # Add more languages...
        }
        ...
        avg_func_lines = code_lines / function_count if function_count > 0 else code_lines
        ...
        raise ValueError(f"Error analyzing complexity in {file_path}: {e}")
