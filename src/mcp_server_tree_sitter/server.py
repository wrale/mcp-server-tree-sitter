...
from .models.ast import node_to_dict, find_node_at_position
from .resources.files import (
    list_project_files, get_file_content, get_file_info, count_lines
)
from .resources.ast import parse_file, get_file_ast
from .tools.project import (
    register_project, get_project, list_projects, remove_project, project_registry
)
from .tools.search import search_text, query_code
from .tools.analysis import (
    extract_symbols, analyze_project_structure,
    find_dependencies, analyze_code_complexity
)
from .tools.query_builder import (
    get_template, build_compound_query, 
    adapt_query_for_language, describe_node_types
)
...
def install_language(language: str) -> Dict[str, str]:
    ...
    if language in language_registry.list_available_languages():
        return {"status": "success", "message": f"Language '{language}' is already installed"}
    
    language_registry.install_language(language)
    return {"status": "success", "message": f"Language '{language}' installed successfully"}
...
def get_syntax_tree_depth_resource(project: str, path: str, depth: int) -> Dict[str, Any]:
...
def suggest_improvements(project: str, file_path: str) -> str:
    ...
    try:
        ...
        - Comment lines: {complexity['comment_lines']} ({complexity['comment_ratio']:.1%})
...
def project_overview(project: str) -> str:
    ...
    try:
        ...
        ) if analysis['entry_points'] else "None detected"
        
        build_files_str = "\n".join(
            f"- {file['path']} ({file['type']})" 
            for file in analysis['build_files']
        ) if analysis['build_files'] else "None detected"
