...
def test_position_inside_function_returns_function_scope(python_tree_and_source):
    """Position inside function body → dict has five keys, kind function/method, name matches, text contains function, start_line <= row <= end_line."""
    ...
    assert set(result.keys()) == {"kind", "text", "start_line", "end_line"}
...
def test_position_on_import_returns_module_scope(python_tree_and_source):
    ...
    # as we have the whole module, functions and classes are there as well
    assert "def foo()" in result["text"] and "return 1" in result["text"]
