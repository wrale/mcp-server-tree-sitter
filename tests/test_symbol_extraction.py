...
from mcp_server_tree_sitter.tools.analysis import get_symbols, get_dependencies
from mcp_server_tree_sitter.tools.project import register_project_tool
from mcp_server_tree_sitter.resources.ast import get_ast
...
""")
        ...
        # Register project
        project_name = "symbol_test_project"
...
def test_symbol_extraction_diagnostics(test_project):
    ...
    # Even though we know these might fail based on FEATURES.md, we're documenting expectations
    if symbols['functions'] and len(symbols['functions']) > 0:
        assert len(symbols['functions']) == expected_function_count
    else:
...
def test_dependency_analysis_diagnostics(test_project):
    ...
    # Even though we know it might fail based on FEATURES.md, document expectations
...
def test_query_based_symbol_extraction(test_project):
    ...
    try:
        ...
        if len(functions) != len(symbols['functions']):
            print(f"ISSUE: Function count mismatch")
            
        if len(classes) != len(symbols['classes']):
            print(f"ISSUE: Class count mismatch")
            
        if len(imports) != len(symbols['imports']):
            print(f"ISSUE: Import count mismatch")
